...
The first of the two was closed in Phase 8: `apps/desktop` exists and the nightly workflow
builds it on macOS and on Windows, which is what the criterion asked for. The second is still
blocked on a host.
...
| P3.10 | Section 19.2, credential brute force                        | The credential routes are throttled per address and route in process memory, ten attempts per fifteen minutes by default, and a success returns the budget. The allowance is configuration (`CREDENTIAL_ATTEMPT_LIMIT`), because one address is one player in a deployment and every player in the browser suite. A single instance is the deployment shape of this phase; a shared store can replace the limiter without touching the routes. | `apps/server/src/identity/rate-limit.ts`, `packages/config/src/schema.ts`, `apps/server/test/identity-api.test.ts` |
...
| P5.3  | Section 13.1, "optional small orbit range, not free rotation"                      | The camera keeps a fixed polar angle of 55 degrees from vertical, allows 18 degrees of azimuth either side of centre, allows a small zoom range around the base distance, and never pans. Playing dark rotates the rig by 180 degrees so the local reserve is nearest. The distance and the zoom range are revised by P5.18.                                                                                                                                                                                                                                                                                                                                                                                                   | `packages/game-ui/src/scene/camera.ts`, `packages/game-ui/test/camera.test.ts`                                                     |
...
| P5.15 | Section 13.3, "Tab through movable pieces" and cursor navigation                   | Tab order is the document order of what can be pressed: the movable pieces and, once one is lifted, its legal destinations. Arrow keys and WASD move the cursor, and browser focus follows the cursor while the board holds focus, so the focus ring and the highlighted square never disagree. Both tiers expose the same focus stops with the same labels, so the WebGL tiers also reach the reserves by keyboard.                                                                                                                                                                                                                                                                                                           | `packages/game-ui/src/interaction/use-cursor-focus.ts`, `packages/game-ui/test/scene-focus.test.tsx`                               |
| P5.16 | Section 2.9, "last two versions of Safari" and keyboard access                     | Safari's engine gives a clicked button no focus and tabs to a button only when macOS full keyboard access is on. The client therefore focuses the stop it was told to act on, so a pointer gesture leaves the keyboard usable in every engine, and the browser suite proves the tab order in the engine that offers it while the unit suite holds it everywhere.                                                                                                                                                                                                                                                                                                                                                               | `packages/game-ui/src/flat/FlatBoard.tsx`, `e2e/tests/keyboard.spec.ts`                                                            |
| P5.17 | Section 13.3, "click a destination to submit" in a perspective scene               | In the WebGL tiers the canvas owns the pointer: a click is resolved by the Three.js hit test, so a piece that occludes a square is picked instead of the square. The DOM stops over the canvas take no pointer event and exist for the keyboard and assistive technology, positioned where the camera projects each square and reserve stack. A pointer gesture then moves focus to the stop it acted on.                                                                                                                                                                                                                                                                                                                      | [ADR-0025](adr/0025-the-canvas-owns-the-pointer.md), `packages/game-ui/test/projection.test.ts`, `e2e/tests/scene-pointer.spec.ts` |
| P5.18 | Section 13.1, "realistic wooden 4×4 board" and four distinguishable sizes          | A piece is a closed surface turned from a profile, with grooves in its wall and an inlaid disc on its top, and the four sizes differ in radius, height, groove count and inlay width so they are told apart at a glance. The grid and both reserve rows sit on one slab, the squares are one wood with the darker slab showing between them, and a highlight marks a square's rim rather than flooding it, so a board on which every square is legal stays readable. The camera is a long lens at 12 units with a 20 degree field of view, aimed 0.4 units past the board centre towards the seated player, and zoom is capped at 1.05 so that no square and no reserve piece leaves the frame at any allowed azimuth or zoom. | `packages/game-ui/src/scene/layout.ts`, `packages/game-ui/test/layout.test.ts`, `packages/game-ui/test/projection.test.ts`         |
| P5.19 | Section 13.3, "hover highlights a movable visible piece. Select lifts it slightly" | Hover only brightens a piece, and only selection lifts one, in every tier. Hover therefore never moves anything, which keeps the two states distinguishable when a pointer passes over a piece the player has already lifted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `packages/game-ui/src/scene/description.ts`, `packages/game-ui/test/scene-description.test.tsx`                                    |

---

## Appendix P6 — Phase 6 decisions and deviations (recorded, not silently decided)
...
| ID    | Specification text                                                          | Decision                                                                                                                                                                                                                                                                                                                                                                                                 | Held by                                                                                                                   |
| ----- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| P6.1  | Section 12, preset messages and reactions                                   | A phrase or a reaction is relayed and forgotten: nothing is written to the database, so there is no message history, no `match_events` row and nothing for a replay to expose. The wire carries a key from a closed set, never words.                                                                                                                                                                    | [ADR-0026](adr/0026-communication-is-relayed-never-stored.md)                                                             |
| P6.2  | Section 12.3 and 12.4, "muted communication must not be rendered or played" | Mute is enforced by withholding: the server does not send an event to a muted recipient, so no client is asked to hide something it already has. The server holds each connection's mute state, seeded from the profile of an account and updated by `match:mute-state`. Sound mute stays entirely local, and a player's own message is always echoed to them.                                           | [ADR-0026](adr/0026-communication-is-relayed-never-stored.md), `apps/server/test/communication.test.ts`                   |
| P6.3  | Section 7.3, `match:mute-state`                                             | The event carries both channels, `presetMessagesMuted` and `reactionsMuted`, rather than the single `muted` flag sketched in `docs/protocol.md`, because section 12.3 requires the channels to be muted independently. The document is corrected.                                                                                                                                                        | [`protocol.md` section 8.1](protocol.md), `packages/protocol/test/communication.test.ts`                                  |
| P6.4  | Section 12.1, "messages may be sent at any time"                            | Any participant of a match may send, guests included, while the match exists: during play and after it ends, so "good game" is sendable when it is meant. A non-participant is refused, and an unknown key is a validation failure rather than a silent no-operation.                                                                                                                                    | [`protocol.md` section 13](protocol.md), `apps/server/test/communication.test.ts`                                         |
| P6.5  | Section 11.4, "Uncovered"                                                   | Earned when the account won a match in which at least one of its own moves revealed an opponent line and blocked it with the same placement. The rules make those the only revealing moves that can be played, since an unblocked reveal loses at once (section 3.8). The fact is recorded on the move event when the rules engine computes it.                                                          | [ADR-0027](adr/0027-achievements-awarded-in-the-completion-transaction.md), `apps/server/test/achievements.test.ts`       |
| P6.6  | Section 11.4, "Four Ways"                                                   | The four categories are a row, a column, the first diagonal and the second diagonal. Only a win by a line counts, which is end reason `line` or `revealed-line`, because a timeout or a resignation has no line. The winning line identifiers are written on the match row as it completes, and the categories accumulate over the account's lifetime.                                                   | [ADR-0027](adr/0027-achievements-awarded-in-the-completion-transaction.md), `apps/server/test/achievements.test.ts`       |
| P6.7  | Section 11.4, "Complete 10 matches" and "Complete 100 matches"              | A completed match is one that reached a result, in either mode; an aborted match counts for nothing. "First Victory" likewise counts a win in either mode, while "Contender", "On a Roll" and "Time Keeper" are ranked by their own wording.                                                                                                                                                             | `apps/server/test/achievements.test.ts`                                                                                   |
| P6.8  | Section 15.7, `badge_asset`                                                 | A badge is a code that the client renders from the design tokens, as the board and the sounds are (ADR-0022). No binary asset enters the repository in this phase, and the column holds that code. Guests earn nothing, having no account; matches claimed from a guest session count towards the account they were moved to.                                                                            | [ADR-0027](adr/0027-achievements-awarded-in-the-completion-transaction.md), `packages/protocol/test/achievements.test.ts` |
| P6.9  | Section 11.3, "daily, weekly, monthly"                                      | Periods are calendar periods in UTC: the day, the ISO week beginning on Monday, and the calendar month containing the request. A global board cannot be paginated coherently if its shape depends on the reader's time zone.                                                                                                                                                                             | [ADR-0028](adr/0028-leaderboards-are-read-time-queries.md), `packages/db/test/leaderboards.test.ts`                       |
| P6.10 | Section 11.3, tie-breaker "earlier achievement of current rating"           | The moment the account's rating last changed, which is `ratings.updated_at`. An account that reached a rating earlier outranks one that reached the same rating later.                                                                                                                                                                                                                                   | [ADR-0028](adr/0028-leaderboards-are-read-time-queries.md), `packages/db/test/leaderboards.test.ts`                       |
| P6.11 | Section 11.3, "all eligible users" and deeper results                       | Eligible means an active account with a rating row, so an account that has never finished a ranked match has no rank rather than an invented one, and suspended or deleted accounts do not appear. The first page is the top hundred, deeper pages use a cursor over the composite sort key rather than an offset, and the caller's own rank comes from the same query definition. Rank is never stored. | [ADR-0028](adr/0028-leaderboards-are-read-time-queries.md), `apps/server/test/leaderboards.test.ts`                       |
| P6.12 | Section 11.1, "recent match summaries" on a public profile                  | The five most recent completed matches, carrying only the fields section 11.2 permits and never the move log. Opponents are shown by the display name recorded on the match, which is what a guest opponent has instead of a username.                                                                                                                                                                   | `apps/server/test/identity-api.test.ts`                                                                                   |
| P6.13 | Section 11.1, "current leaderboard rank"                                    | The all-time rank, since that is the board every rated account belongs to. Period ranks are shown on the leaderboard screen, where the period is chosen, and an account with no rating shows no rank at all.                                                                                                                                                                                             | `apps/server/test/identity-api.test.ts`                                                                                   |
| P6.14 | Section 13.5, the reaction sound                                            | The reaction sound is now played by an incoming reaction, which appendix P5.6 recorded as unused. A muted reaction plays nothing because the event is never delivered, rather than because the client suppresses it.                                                                                                                                                                                     | `apps/web/test/match-communication.test.tsx`                                                                              |

---

## Appendix P7 — Phase 7 decisions and deviations (recorded, not silently decided)
...
| ID    | Specification text                                                              | Decision                                                                                                                                                                                                                                                                                                                                                                                                                              | Held by                                                                                                               |
| ----- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| P7.1  | Section 16, "protected admin dashboard"                                         | An administrator is an ordinary account carrying the `admin` role, and the dashboard is a set of gated routes in the player client rather than a second application. Every `/v1/admin/*` route checks the role on the account on every request, and refuses with the same `403` for a player, a guest and an anonymous caller.                                                                                                        | [ADR-0029](adr/0029-administration-is-a-role-on-the-account.md)                                                       |
| P7.2  | Section 16, "Search users by ... email-derived identity lookup where permitted" | Search matches a username prefix, an internal id, or an email address in full. A partial address matches nothing, so the dashboard cannot be used to enumerate addresses, and an address is shown only on the detail page of a single account. Password hashes and session tokens are never returned.                                                                                                                                 | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.3  | Section 16, "Create, edit, enable, and disable achievements"                    | The catalogue is metadata over rules that live in code, keyed by code (ADR-0027). Creation is limited to codes the server has a rule for, so nothing unearnable can be offered; editing covers the name, the description and the badge tier; enabling and disabling is a flag. Nothing is deleted, because accounts may already hold the badge, and a rule change bumps `rule_version`.                                               | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.4  | Section 16, "Correct Elo with required reason"                                  | A correction writes the new rating, a `rating_adjustments` row and an audit row in one transaction. It is not a `rating_changes` row: that table is the per-match rating audit period leaderboards aggregate, and a correction has no match, no side and no opponent. The account's `updated_at` moves, because its rating really did.                                                                                                | [ADR-0029](adr/0029-administration-is-a-role-on-the-account.md), `apps/server/test/admin-api.test.ts`                 |
| P7.5  | Section 16, "Inspect complete internal match event history"                     | Inspection returns every `match_events` row with its payload, state hash and clock state, plus the connection history from its own table. A socket attaching or detaching is not a match event, because an event consumes the sequence number that is the match version.                                                                                                                                                              | [ADR-0029](adr/0029-administration-is-a-role-on-the-account.md), `apps/server/test/admin-api.test.ts`                 |
| P7.6  | Section 16, "View active matches" and "queue depth by mode and time control"    | Both are read from the process that owns them, the match runtime and the in-process queues (ADR-0018), at the moment of the request. A single instance is the deployment shape of this phase (ADR-0015); the shared store those interfaces anticipate is what a second instance would need.                                                                                                                                           | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.7  | Section 16, "View recent errors"                                                | The dashboard shows error counts by code from the metrics registry and the most recent reported errors from a bounded in-process ring, carrying the code, the route and the moment. Messages and stack traces are not shown, since the error reporter, not the dashboard, is where a stack belongs.                                                                                                                                   | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.8  | Section 16, "View daily active users"                                           | Distinct accounts and guest sessions last seen inside the twenty-four hours before the request, counted separately and summed, since a guest is an active user without an account.                                                                                                                                                                                                                                                    | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.9  | Section 16, "match completion and abandonment rates"                            | Over the same window: the completion rate is matches that reached a result divided by matches that reached a result or were aborted, and the abandonment rate is its complement. A match still being played counts in neither, so the two rates describe finished matches rather than a moving denominator.                                                                                                                           | `apps/server/test/admin-api.test.ts`                                                                                  |
| P7.10 | Section 16 and 17.3, "desktop version adoption"                                 | The metric and the summary field exist and stay empty until a desktop client does, in Phase 8. The client version comes from the socket handshake, which already carries it for the minimum-version check.                                                                                                                                                                                                                            | [`operations.md` section 3](operations.md)                                                                            |
| P7.11 | Section 17.1, the analytics event list                                          | Events are a closed schema, the client sends its own through the server, and the identity on every event is a keyed pseudonym. "Authentication method" is always `password`, since that is the only method (ADR-0017), and the desktop update events wait for Phase 8.                                                                                                                                                                | [ADR-0030](adr/0030-telemetry-behind-ports-relayed-through-the-server.md), `packages/protocol/test/telemetry.test.ts` |
| P7.12 | Section 17.2, "user or guest pseudonymous ID"                                   | One keyed hash serves logs and analytics, so a line and an event can be correlated without either naming an account. Rotating `TELEMETRY_PSEUDONYM_SECRET` detaches new records from old ones, which is the point of a pseudonym rather than a defect.                                                                                                                                                                                | [ADR-0030](adr/0030-telemetry-behind-ports-relayed-through-the-server.md), `apps/server/test/telemetry.test.ts`       |
| P7.13 | Section 2.8 and 17, Sentry and product analytics                                | Both sit behind ports with one transport each, `@sentry/node` and `posthog-node`, and both are inert without a DSN or an API key, which is how the suites and a developer machine run. No provider software reaches the browser.                                                                                                                                                                                                      | [ADR-0030](adr/0030-telemetry-behind-ports-relayed-through-the-server.md), `apps/server/test/telemetry.test.ts`       |
| P7.14 | Section 17.3, the metric list                                                   | A Prometheus text exposition from `prom-client` on `GET /metrics`, absent unless `METRICS_ENABLED` is set and requiring a bearer token when `METRICS_TOKEN` is set. Labels are bounded vocabularies, never a user or match id. The dashboard summary is SQL over the product tables instead, so a deployment does not reset it.                                                                                                       | [ADR-0031](adr/0031-metrics-are-a-prometheus-exposition.md), `apps/server/test/metrics.test.ts`                       |
| P7.15 | Section 17.4, "Alert on ..."                                                    | Each condition is a rule over the exposition in `ops/alerts/gobblet.rules.yml`, and each rule is exercised by a test that drives the metrics into the failing state and evaluates the expression. Delivering an alert to a human needs the hosted monitoring of ADR-0015 and is deferred with it; the conditions are not deferred.                                                                                                    | [ADR-0031](adr/0031-metrics-are-a-prometheus-exposition.md), `apps/server/test/alert-rules.test.ts`                   |
| P7.16 | Section 22.2, "Deploy to staging ... require approval for production"           | The workflow exists with its ordered jobs, the approval gate and the drain step, and it is triggered manually because there is no host to deploy to. Drain and reconnect behaviour is proved by a test against the runtime rather than by a deployment, which is what section 7.6 actually constrains.                                                                                                                                | `.github/workflows/deploy.yml`, `apps/server/test/phase7-exit-criteria.test.ts`                                       |
| P7.17 | Section 23, backups and disaster recovery                                       | The export, the restore and the verification are scripts in the repository, and the round trip runs in continuous integration against a real PostgreSQL, comparing a restored match with the original. Managed daily backups, point-in-time recovery, retention, the encrypted upload and the quarterly drill belong to the managed service and are named as deferred in the runbook.                                                 | [ADR-0032](adr/0032-backups-are-scripts-proved-by-a-restore.md), `packages/db/test/backup-restore.test.ts`            |
| P7.18 | Section 14.4, "Every administrative mutation requires an audit record"          | The audit row is written in the same transaction as the change, so a failed mutation leaves no record and no change leaves no record. The reason is required by the schema rather than by the screen, and no endpoint edits or deletes an audit row. The first administrator is granted by a script, since nothing in the API may create one.                                                                                         | [ADR-0029](adr/0029-administration-is-a-role-on-the-account.md), `apps/server/test/phase7-exit-criteria.test.ts`      |
| P7.19 | `architecture.md` section 3, the planned `packages/observability`               | The package is not created. Logging, the metrics registry, the pseudonym and both provider ports live in `apps/server/src/observability`, because the only consumer is the server and a package that one application imports is a directory with extra ceremony. The web client's reporter is separate and smaller: it posts to the relay. If a second Node process ever needs the registry, extracting it is a move, not a redesign. | `apps/server/src/observability/*`, [`architecture.md` section 3](architecture.md)                                     |

## Appendix P8 — Phase 8 decisions and deviations (recorded, not silently decided)
...
| ID    | Specification text                                                                                                                 | Decision                                                                                                                                                                                                                                                                                                                                                                                                                  | Held by                                                                                                             |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| P8.1  | Section 5.4, "Open external authentication in the system browser" and "Receive OAuth callback through a registered deep link"      | Void, and recorded as void rather than skipped. ADR-0017 replaced the hosted identity provider with first-party email and password authentication, so there is no external login page to open and no callback to receive; the desktop signs in against the same API as the browser. Appendix P3 already records the matching Phase 3 criterion as void. If a hosted provider ever returns, the deep link returns with it. | [ADR-0033](adr/0033-the-desktop-application-is-the-web-build-in-a-window.md)                                        |
| P8.2  | Section 5.4, "Store refresh/session material using platform secure storage where supported"                                        | There is no refresh token; the session is one opaque bearer string. On the desktop it is held in the operating system's credential store through three Rust commands, and the client's existing injected key-value store is what selects between that and `localStorage`. The store is read once before the first render because the commands are asynchronous.                                                           | [ADR-0033](adr/0033-the-desktop-application-is-the-web-build-in-a-window.md), `apps/web/test/desktop.test.ts`       |
| P8.3  | Section 22.3, "Publish update manifest" and "Ability to pause update rollout"                                                      | The manifest is served by our own API from a release row rather than published as a static file, so pausing, promoting and pulling a release are audited administrative mutations instead of edits to a published asset. Promotion never rebuilds or resigns: what the beta channel tested is what stable receives.                                                                                                       | [ADR-0034](adr/0034-updates-are-asked-of-our-own-server.md), `apps/server/test/updates-api.test.ts`                 |
| P8.4  | Section 19.2, "Signed update manifests and bundles"                                                                                | The bundle signature is Tauri's detached minisign signature, produced on the build machine and verified in the application against the public key compiled into it. The manifest itself is served over HTTPS from an authenticated origin and is not separately signed, because a forged manifest cannot produce an installable bundle: the signature check is on the bytes that are installed.                           | [ADR-0034](adr/0034-updates-are-asked-of-our-own-server.md)                                                         |
| P8.5  | Section 24 (Phase 8), "macOS signing", "macOS notarization", "Windows signing"                                                     | The workflow steps exist in full and each one stops the release with the name of the missing secret. No path produces a signed-looking artifact without an identity, and an explicitly unsigned developer build cannot be published as a release.                                                                                                                                                                         | [ADR-0036](adr/0036-signing-is-a-workflow-step-that-fails-loudly.md), `.github/workflows/desktop-release.yml`       |
| P8.6  | Phase 8 exit criteria, "Clean macOS machine installs and launches without security warning" and the Windows SmartScreen equivalent | Deferred, with the remaining steps named: an Apple Developer Program membership, a Developer ID Application certificate and an app-specific password for notarization; an organisation-validated or extended-validation Windows code-signing certificate, after which SmartScreen reputation accrues with downloads and cannot be tested before publication. Both plug into steps that already exist in the workflow.     | [ADR-0036](adr/0036-signing-is-a-workflow-step-that-fails-loudly.md), [`operations.md` section 13](operations.md)   |
| P8.7  | Phase 8 exit criterion, "Prior version updates successfully to release candidate"                                                  | Proved as far as an unsigned installation allows: a test drives the update client against a real manifest endpoint and asserts that a lower version is offered the higher one, that an equal or higher version is offered nothing, that a paused release is offered nothing, and that the artifact digest and signature reach the updater unchanged. Installing the downloaded bundle on a clean machine is part of P8.6. | `apps/server/test/updates-api.test.ts`, `apps/web/test/desktop-update.test.ts`                                      |
| P8.8  | Phase 8 exit criterion, "Failed update leaves prior application usable"                                                            | The update client treats every failure, network, digest, signature or install, as a report and a dismissal: nothing is written to the running installation until the bundle verifies, which is Tauri's atomic replacement. The client's half is tested by driving each failure; the atomic replacement is the updater's own guarantee and is named as such rather than claimed as ours.                                   | `apps/web/test/desktop-update.test.ts`                                                                              |
| P8.9  | Section 5.4, "Confirm before closing an active match"                                                                              | The interception lives in the client behind the desktop predicate, not in Rust: closing during an active match asks whether quitting should resign, and confirming sends `match:resign` and waits for the acknowledgement before the window closes. A player with no active match closes immediately.                                                                                                                     | [ADR-0033](adr/0033-the-desktop-application-is-the-web-build-in-a-window.md), `apps/web/test/desktop-close.test.ts` |
| P8.10 | Section 5.4, "Report native crashes and update failures"                                                                           | Update failures are reported through the telemetry relay of ADR-0030 as ordinary client events, so no key reaches the desktop. Native crashes of the shell process itself are not reported: a crash reporter would need a provider and a symbol upload pipeline, and the shell's own code is three commands and a window. Web-view errors were already reported in Phase 7.                                               | [ADR-0030](adr/0030-telemetry-behind-ports-relayed-through-the-server.md)                                           |
| P8.11 | Section 17.4, "Desktop update-signing failure"                                                                                     | The series exists now: a failing signing or notarization step reports a build event to the API before the job fails, and the server counts it. The `GobbletDesktopSigningFailure` rule is no longer pending.                                                                                                                                                                                                              | [ADR-0036](adr/0036-signing-is-a-workflow-step-that-fails-loudly.md), `apps/server/test/alert-rules.test.ts`        |
| P8.12 | Section 24 (Phase 8), "Production icons and metadata"                                                                              | The icons are generated from the same procedural source as the rest of the placeholder art (ADR-0022): a rendered board mark, not commissioned artwork. They are complete, correctly sized for both platforms and committed, and replacing them is a file swap and a rebuild.                                                                                                                                             | [ADR-0022](adr/0022-procedural-placeholder-assets.md), `apps/desktop/src-tauri/icons`                               |
| P8.13 | Section 24 (Phase 8), "Direct-download pages"                                                                                      | One route in the web client, `/download`, which reads the same release records the updater reads and offers the current stable build for the visitor's platform with its size and digest, the other platform beside it, and the release notes. A separate marketing site is not in the specification's package list.                                                                                                      | `apps/web/src/screens/DownloadScreen.tsx`, `e2e/tests/download.spec.ts`                                             |
| P8.14 | Section 5.9, "Object storage or GitHub Releases"                                                                                   | GitHub Releases, because no object storage exists until a provider is chosen (ADR-0015). The schema records a URL per platform, so a mirror is another row rather than a redesign. Anonymous downloads require the repository to be public, which is a launch step in the runbook.                                                                                                                                        | [ADR-0035](adr/0035-artifacts-live-in-github-releases.md)                                                           |
| P8.15 | Section 22.3, "Test staged update channel" then "Promote manifest to stable"                                                       | The workflow publishes to `beta`, runs a staged check that asks the endpoint as a beta client would and verifies the manifest it receives, and only then promotes. Promotion is an admin call that changes a channel, so the artifacts are untouched.                                                                                                                                                                     | `.github/workflows/desktop-release.yml`, `apps/server/test/updates-api.test.ts`                                     |
| P8.16 | Section 22.3, the release workflow                                                                                                 | Everything the workflow does beyond ordering steps and holding secrets lives in `apps/server/src/ops/desktop-release.ts` and is covered by tests, with `pnpm --filter @gobblet/server desktop-release <step>` as the entry point. A release step that only ever runs during a release is a step nobody has proved, which is the same reason the deploy smoke checks are a module rather than a shell block.               | `apps/server/test/desktop-release.test.ts`, `.github/workflows/desktop-release.yml`                                 |
| P8.17 | Phase 0 exit criterion, "Empty Tauri shells build on macOS and Windows CI"                                                         | Met here rather than in Phase 0, and nightly rather than per push: a cold Rust build costs minutes on both hosted platforms, and nothing in an ordinary change can break the shell without also breaking the web build it packages. A tagged release builds all three targets anyway.                                                                                                                                     | `.github/workflows/nightly.yml` > `desktop-shell`, appendix P0                                                      |

## Appendix P9 — Phase 9 decisions and deviations (recorded, not silently decided)
...
| ID    | Specification text                                                                                  | Decision                                                                                                                                                                                                                                                                                                                                                                       | Held by                                                                                                                |
| ----- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| P9.1  | Section 20.8, "Use k6, Artillery, or equivalent"                                                    | Equivalent, and ours. The load target is about protocol behaviour under concurrency, not request throughput, so the harness drives real sessions through `@gobblet/protocol` and asserts acknowledgement latency, lost moves and duplicate completions. No new tool enters the toolchain.                                                                                      | [ADR-0037](adr/0037-the-load-harness-is-ours.md), `apps/server/test/load.test.ts`                                      |
| P9.2  | Section 20.8, "1,000 simultaneous connected clients" and "500 simultaneous active matches"          | Deferred against a host, with the command that runs it recorded. What runs here is the same harness at the size a shared runner can carry: named a host it drives that, and named none it starts a server of its own so the gate needs no deployment. The report always states the scale it ran at, so a small run can never be read as the baseline.                          | [ADR-0037](adr/0037-the-load-harness-is-ours.md), [`operations.md` section 16](operations.md)                          |
| P9.3  | Section 21, "every quality gate passes"                                                             | The twenty-five gates are one typed definition executed by `pnpm gates`. A gate is executable, or it is deferred with the reason and the thing it waits for; nothing is reported as passing because nobody ran it. A test asserts the definition covers every clause of sections 21.1 and 21.2.                                                                                | [ADR-0038](adr/0038-quality-gates-are-a-definition-not-a-checklist.md), `apps/server/test/gates.test.ts`               |
| P9.4  | Section 21.2, "Zero known critical defects" and "Zero known high-severity defects"                  | Executable, by reading `docs/defects.md`. The register is strict: an unreadable row is an error rather than a skipped row, and accepting a defect at critical or high still fails the gate.                                                                                                                                                                                    | [ADR-0039](adr/0039-the-defect-register-is-a-gate.md), `apps/server/test/defects.test.ts`                              |
| P9.5  | Section 24 (Phase 9), "Privacy policy" and "Terms"                                                  | Routes in the client at `/privacy`, `/terms` and `/support`, rendered from a typed content module, linked from a footer on every screen and shipped with the desktop build. The pages state that they have not been reviewed by a lawyer and that the operator's legal identity is a placeholder until one exists.                                                             | [ADR-0040](adr/0040-legal-and-support-pages-are-routes-in-the-client.md), `apps/web/test/legal-pages.test.tsx`         |
| P9.6  | Section 19.2, "Secure, HttpOnly, SameSite cookies where cookies are used"                           | No cookies are used at all, so the clause is satisfied vacuously and no consent banner is shown. The session is a bearer token in `localStorage` in a browser and in the operating system credential store on the desktop, and the privacy page lists every storage key with the reason it is strictly necessary.                                                              | [ADR-0040](adr/0040-legal-and-support-pages-are-routes-in-the-client.md), `apps/web/test/legal-pages.test.tsx`         |
| P9.7  | Section 20.7, the manual rendering matrix                                                           | Firefox becomes a third Playwright project running nightly rather than a promise of a manual pass; Edge is Chromium and is recorded as covered by it rather than duplicated; Safari, the packaged desktop web views and the GPU rows stay manual, dated, and listed in `docs/compatibility.md`.                                                                                | [ADR-0041](adr/0041-the-compatibility-matrices-are-executed-where-a-runner-exists.md), `.github/workflows/nightly.yml` |
| P9.8  | Section 24 (Phase 9), "Launch dashboards"                                                           | Three dashboards defined once in TypeScript and rendered to Grafana JSON by `pnpm ops:dashboards`, with a test asserting every series a panel names is one the server actually emits. Importing them into an instance is deferred with the host.                                                                                                                               | [ADR-0042](adr/0042-launch-dashboards-are-rendered-from-one-definition.md), `apps/server/test/dashboards.test.ts`      |
| P9.9  | Section 21.2, "Rollback procedure tested"                                                           | Tested as far as a repository without a host allows: the workflow has a rollback path that redeploys a named earlier commit and re-runs the smoke check, and a test proves the mechanism that makes a rollback verifiable, which is that the smoke check fails when the version serving is not the version expected.                                                           | `.github/workflows/deploy.yml`, `apps/server/test/phase9-exit-criteria.test.ts`                                        |
| P9.10 | Section 21.2, "Error-monitoring release marker present"                                             | The release marker is the version and commit the server reports on `/v1/config` and stamps on every error report and analytics event, which is what a provider needs to attribute a regression to a release. Creating a release object in a provider's API is deferred with the provider's key.                                                                                | `apps/server/src/observability/telemetry.ts`, `apps/server/test/phase9-exit-criteria.test.ts`                          |
| P9.11 | Section 21.1, "Dependency vulnerability threshold passes" and "No secrets detected"                 | Both become steps in the pull-request workflow: `pnpm audit` at the high threshold, and a scanner in the repository that reads the tracked files. The scanner carries a small allowlist with a reason per entry, one of which is the Tauri update public key, which is public by design and must remain committed.                                                             | `apps/server/src/ops/secret-scan.ts`, `apps/server/test/secret-scan.test.ts`, `.github/workflows/ci.yml`               |
| P9.12 | Section 24 (Phase 9), "Product owner approves visual quality" and "approves official-rule behavior" | Deferred, because they are judgements by a person and cannot be asserted by a test. What is prepared for them is recorded: the rule behaviour is enumerated against the official rules in `docs/rules.md` and proved by the game-core suite, and the visual pass has a scripted route list in the launch checklist so the approval is a review of the same screens every time. | [`operations.md` section 17](operations.md), `docs/rules.md`                                                           |
| P9.13 | Section 24 (Phase 9), "Production readiness review is signed off"                                   | Deferred with the host. The review's checklist exists as the launch checklist in the operations runbook, every item is marked executable or blocked, and the blocked items name the same missing host as ADR-0015.                                                                                                                                                             | [`operations.md` section 17](operations.md)                                                                            |

## Appendix L — Launch preparation decisions (recorded, not silently decided)

The phases are delivered. What remains before a public launch is the work that needs a purchase,
a host or a person, enumerated in [`launch-blockers.md`](launch-blockers.md). Decisions taken
while clearing those items are recorded here, in the same form as the phase appendices. Where an
entry supersedes a phase appendix, it says so; the earlier entry stays as it was written.

| ID  | Specification text                                                   | Decision                                                                                                                                                                                                                                                                                                                                                                                       | Held by                                                                                                                                |
| --- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| L1  | Section 22.2, "Build immutable server image" and "Deploy to staging" | The host is Railway, one project, one region, one replica per service, and the unit of deployment is a Dockerfile this repository owns rather than a provider buildpack. Staging is deferred until production exists, so the deploy workflow takes an explicit `skip-staging` input and the approval records a release as untried rather than rehearsed. Supersedes the deferral in P7.16.     | [ADR-0043](adr/0043-railway-hosts-the-deployment.md), `apps/server/test/container.test.ts`, `apps/server/test/deploy-workflow.test.ts` |
| L2  | Section 22.2, "Deploy production with drain-and-reconnect strategy"  | The drain window becomes real configuration, `SHUTDOWN_DRAIN_SECONDS`, applied on `SIGTERM`: matchmaking closes at once, matches in flight are given the window, then the sockets and the pool close. The image starts `node` directly, because a package manager as PID 1 swallows the signal, and the platform's own draining period is set above the window so nothing is killed mid-drain. | `apps/server/src/shutdown.ts`, `apps/server/test/shutdown.test.ts`, `apps/server/railway.json`                                         |
| L3  | Section 22.2, "Run smoke tests" after a deploy                       | A provider's release command returns when the build finishes, which is not when the build serves. The workflow therefore waits for the released version to answer `GET /health/live` before smoking it, and the wait is a tested module with an injected clock rather than a sleep.                                                                                                            | `apps/server/src/ops/release.ts`, `apps/server/test/release-wait.test.ts`                                                              |
