...
| P5.5  | Section 2.7 and section 22, hidden pieces                          | Every tier draws only the top piece of a stack. A covered piece is never drawn, never named and never present in the accessible output, although the snapshot contains it, because the client needs it to compute the reveal warning the specification requires.                                                            | `packages/game-ui/test/flat-board.test.ts`, `e2e/tests/hidden-pieces.spec.ts`                            |
| P5.6  | Section 13.5, "reaction" sound                                     | The communication channel and its volume control are delivered, but no reaction sound plays yet, because reactions and preset messages are Phase 6. The channel is exercised by the match-found notification until then.                                                                                                    | `packages/game-ui/src/sound/engine.ts`, `packages/game-ui/test/sound.test.ts`                            |
