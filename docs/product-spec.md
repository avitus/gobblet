...
| P6.12 | Section 11.1, "recent match summaries" on a public profile                  | The five most recent completed matches, carrying only the fields section 11.2 permits and never the move log. Opponents are shown by the display name recorded on the match, which is what a guest opponent has instead of a username.                                                                                                                                                                   | `apps/server/test/profiles.test.ts`                                                                                       |
| P6.13 | Section 11.1, "current leaderboard rank"                                    | The all-time rank, since that is the board every rated account belongs to. Period ranks are shown on the leaderboard screen, where the period is chosen, and an account with no rating shows no rank at all.                                                                                                                                                                                             | `apps/server/test/profiles.test.ts`                                                                                       |
