...
| Empty Tauri shells build on macOS and Windows CI  | **Met in Phase 8**: `.github/workflows/nightly.yml` > `desktop-shell` builds `apps/desktop` on macOS and Windows every night, and the release workflow builds all three targets on a tag        |
...
## 27. Phase 8 exit criteria (spec 24, as amended by appendix P8)

Test roots: `apps/server/test`, `apps/web/test`, `e2e/tests`.

| Criterion                                                    | Evidence                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Clean macOS machine installs without a security warning      | **Deferred**, with the remaining steps named: an Apple Developer Program membership, a Developer ID Application certificate and an app-specific password. The workflow step that consumes them exists and stops the release without them: `phase8-exit-criteria.test.ts` > `stops the release when a signing identity is missing, naming what is needed`; appendix P8.6 |
| Clean Windows machine installs without a SmartScreen warning | **Deferred**: an organisation-validated or extended-validation certificate, after which reputation accrues with downloads and cannot be tested before publication. Same workflow step and same test; appendix P8.6                                                                                                                                                      |
| Prior version updates successfully to the release candidate  | `phase8-exit-criteria.test.ts` > `offers the newer build, with the signature and digest the build machine produced`, `offers nothing to a client that already has it, and nothing to a newer one`, `offers nothing while the rollout is paused, and again once it resumes`; `apps/web/test/desktop-update.test.ts`; appendix P8.7                                       |
| Failed update leaves the prior application usable            | `phase8-exit-criteria.test.ts` > `counts the failure and keeps offering the same build to the same client`; `apps/web/test/desktop-update.test.ts` drives every failure the client can meet; appendix P8.8                                                                                                                                                              |

The Phase 8 deliverables, and what holds each one:

| Specification requirement                                | Where held                                                                                                                                                                                                         |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Section 24: production icons and metadata                | `apps/desktop/scripts/draw-icon.mjs` renders the source, `tauri icon` derives every size into `apps/desktop/src-tauri/icons`; appendix P8.12                                                                       |
| Section 24: macOS signing, notarization, Windows signing | `.github/workflows/desktop-release.yml` bundle job; `phase8-exit-criteria.test.ts` > the deferred-criteria suite; [ADR-0036](adr/0036-signing-is-a-workflow-step-that-fails-loudly.md); appendix P8.5              |
| Section 24: direct-download pages                        | `apps/web/src/screens/DownloadScreen.tsx`; `apps/web/test/download-screen.test.tsx`; `e2e/tests/download.spec.ts`; appendix P8.13                                                                                  |
| Section 24: automatic updater                            | `apps/web/src/desktop/update-client.ts` over `apps/desktop/src-tauri` and the updater plugin; `apps/web/test/desktop-update.test.ts`; [ADR-0033](adr/0033-the-desktop-application-is-the-web-build-in-a-window.md) |
| Section 24: stable and beta channels                     | `packages/db/migrations/0005_phase8_desktop_releases.sql`, `apps/server/src/releases/service.ts`; `updates-api.test.ts`; [ADR-0034](adr/0034-updates-are-asked-of-our-own-server.md)                               |
| Section 24: update failure recovery                      | `apps/web/src/desktop/update-client.ts` reports and dismisses; the rollout pause is an audited mutation; `updates-api.test.ts` > the pause suite; [`operations.md` section 13](operations.md)                      |
| Section 24: client minimum-version control               | `MIN_SUPPORTED_CLIENT_VERSION` refused at the handshake through one shared comparison, `packages/protocol/src/release.ts` `isClientVersionSupported`; `socket-gateway.test.ts`                                     |
| Section 5.4: platform secure storage                     | `apps/desktop/src-tauri/src/lib.rs` three keyring commands, `apps/web/src/desktop/keyring-store.ts`; `apps/web/test/desktop.test.ts`, `desktop-bridge.test.tsx`; appendix P8.2                                     |
| Section 5.4: confirm before closing an active match      | `apps/web/src/desktop/close-guard.ts`, `useCloseGuard.ts`; `apps/web/test/desktop-close.test.tsx`; appendix P8.9                                                                                                   |
| Section 5.4: report update failures                      | `desktop-update-completed` through the telemetry relay of [ADR-0030](adr/0030-telemetry-behind-ports-relayed-through-the-server.md); `telemetry-api.test.ts`; appendix P8.10                                       |
| Section 5.9: artifact storage                            | GitHub Releases, uploaded by the bundle job; [ADR-0035](adr/0035-artifacts-live-in-github-releases.md); appendix P8.14                                                                                             |
| Section 19.2: signed update manifests and bundles        | The detached minisign signature is required by `releaseArtifactSchema` and verified in the application; `packages/protocol/test/release.test.ts`; appendix P8.4                                                    |
| Section 22.3: staged channel then promotion              | `.github/workflows/desktop-release.yml` publish and promote jobs over `apps/server/src/ops/desktop-release.ts`; `desktop-release.test.ts`; appendix P8.15                                                          |
| Section 17.4: desktop update-signing failure             | The failing step reports a build event, the server counts it; `alert-rules.test.ts`, `updates-api.test.ts`; appendix P8.11                                                                                         |

## 28. Phase 9 exit criteria (spec 24, as amended by appendix P9)

Test roots: `apps/server/test`, `apps/web/test`, `e2e/tests`.

| Criterion                                     | Evidence                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Every quality gate in section 21 passes       | `pnpm gates release-candidate` over the twenty-five definitions in `apps/server/src/ops/gates.ts`; `gates.test.ts` asserts every clause of sections 21.1 and 21.2 is a definition and that the four deferred gates name what they wait for; `phase9-exit-criteria.test.ts` > the gate suite; [ADR-0038](adr/0038-quality-gates-are-a-definition-not-a-checklist.md) |
| Zero known critical or high-severity defects  | `pnpm ops:defects` over [`defects.md`](defects.md); `defects.test.ts`; `phase9-exit-criteria.test.ts` > `holds, against the register that ships` and `is a gate, not a claim: an open high-severity defect would fail it`; [ADR-0039](adr/0039-the-defect-register-is-a-gate.md)                                                                                    |
| Product owner approves visual quality         | **Deferred**, because it is a judgement by a person. What is prepared: the scripted route list, viewports and themes in [`operations.md` section 17.2](operations.md), so the review is the same review every time; `phase9-exit-criteria.test.ts` > `are named in the launch checklist, with what each reviewer looks at`; appendix P9.12                          |
| Product owner approves official-rule behavior | **Deferred**, same reason. What is prepared: every rule is stated in [`rules.md`](rules.md) and asserted by the `@gobblet/game-core` suite, and the four rules a player feels are listed for a two-window play-through in section 17.2; appendix P9.12                                                                                                              |
| Production readiness review is signed off     | **Deferred** with the host. The checklist the review will use is section 17.1, every blocked item names its blocker in section 17.3, and the blockers are the same missing host as [ADR-0015](adr/0015-single-region-deployment.md); appendix P9.13                                                                                                                 |

The Phase 9 deliverables, and what holds each one:

| Specification requirement                             | Where held                                                                                                                                                                                                                                                                             |
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Section 24: full regression suite                     | `pnpm gates release-candidate` runs it; the suite is every package's `test:coverage` plus the browser matrix, ordered by `turbo`                                                                                                                                                       |
| Section 24 and 20.8: load tests                       | `apps/server/src/ops/load.ts` and `load-socket.ts`, run by `pnpm load`; `load.test.ts`, `load-integration.test.ts` against a bootstrapped server; the stated scale is deferred and the report says so; [ADR-0037](adr/0037-the-load-harness-is-ours.md); appendix P9.1, P9.2           |
| Section 24: browser matrix                            | Chromium, WebKit and Firefox as three Playwright projects; `e2e/playwright.config.ts`; the nightly `firefox-compatibility` job; [`compatibility.md`](compatibility.md); appendix P9.7                                                                                                  |
| Section 24: desktop compatibility matrix              | [`compatibility.md`](compatibility.md), executed where a runner exists and marked `Not yet run` where a person is needed; `phase9-exit-criteria.test.ts` > `date every row a person has to run`; [ADR-0041](adr/0041-the-compatibility-matrices-are-executed-where-a-runner-exists.md) |
| Section 24 and 18.5: privacy policy and terms         | `apps/web/src/legal/content.ts` rendered at `/privacy`, `/terms` and `/support`, linked from the footer on every screen; `apps/web/test/legal-pages.test.tsx`; [ADR-0040](adr/0040-legal-and-support-pages-are-routes-in-the-client.md); appendix P9.5                                 |
| Section 24: support and incident workflow             | `/support` names the address and what to include; the incident side is [`operations.md` section 11](operations.md); appendix P9.5                                                                                                                                                      |
| Section 24: launch dashboards                         | `apps/server/src/observability/dashboards.ts` rendered to `ops/dashboards/*.json` by `pnpm ops:dashboards`; `dashboards.test.ts` checks every panel's series against a real exposition; [ADR-0042](adr/0042-launch-dashboards-are-rendered-from-one-definition.md); appendix P9.8      |
| Section 24: rollback test                             | The `rollback` input to `.github/workflows/deploy.yml` skips the migrations and proves the result with `APP_VERSION`; `phase9-exit-criteria.test.ts` > the rollback suite; [`operations.md` section 9](operations.md); appendix P9.9                                                   |
| Section 24: release candidate bug burn-down           | [`defects.md`](defects.md) is the burn-down and `pnpm ops:defects` is the gate; each open entry names its severity, its owner area and what unblocks it                                                                                                                                |
| Section 21.2: no critical or high dependency advisory | The `Dependency advisories` step in `.github/workflows/ci.yml` runs `pnpm audit --audit-level high --prod`; the gate definition names that step so the two cannot drift; appendix P9.11                                                                                                |
